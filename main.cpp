//=====================================||
// CacaoLang 1.0.0 - Cacao Crunch      ||
// CoffeeShop Development              ||
// Made by Camila "Mocha" Rose         ||
// g++ -o cocoa main.cpp choco_gui.cpp ||
...
#include <functional>
    ...
    public:
        ...
        void setInterpreter(void* interp) {}
...
bool hasGTK4() {
#ifdef _WIN32
    return system("pkg-config --exists gtk4 >nul 2>&1") == 0;
#else
    return system("pkg-config --exists gtk4 >/dev/null 2>&1") == 0;
#endif
}
...
void showCompileHelp() {
    std::cout << "ChocComp v1.0.0" << std::endl;
    std::cout << "Usage: choco compile <file.choco> [options]" << std::endl;
    std::cout << std::endl;
    std::cout << "Options:" << std::endl;
    std::cout << "  -o <name>      Specify output filename (without extension)" << std::endl;
    std::cout << "  --no-gui       Compile without GUI support (no GTK4 required)" << std::endl;
    std::cout << "  -h, --help     Show this help message" << std::endl;
    std::cout << std::endl;
    std::cout << "Examples:" << std::endl;
    std::cout << "  choco compile myapp.choco" << std::endl;
    std::cout << "  choco compile myapp.choco -o myprogram" << std::endl;
    std::cout << "  choco compile myapp.choco --no-gui" << std::endl;
}
...
bool compileChocoFile(const std::string& inputFile, const std::string& outputName, bool noGUI) {
    std::cout << "ChocComp v1.0.0" << std::endl;
    ...
    // Auto-detect GUI support if not explicitly disabled
    bool useGUI = !noGUI;
    if (useGUI && !hasGTK4()) {
        std::cout << "Warning: GTK4 not found. Compiling without GUI support." << std::endl;
        std::cout << "         GUI functions will be unavailable at runtime." << std::endl;
        useGUI = false;
    }
    
    if (useGUI) {
        std::cout << "Mode: With GUI support (GTK4)" << std::endl;
    } else {
        std::cout << "Mode: Console only (no GUI)" << std::endl;
    }
    
    // Read the Choco source
    std::ifstream input(inputFile);
    std::stringstream buffer;
    buffer << input.rdbuf();
    std::string chocoSource = buffer.str();
    
    // Create temporary C++ file
    std::string tempCpp = "temp_choco_compile.cpp";
    std::ofstream output(tempCpp);
    
    // Write wrapper C++ code
    output << "// Auto-generated by ChocComp\n";
    output << "// Source: " << inputFile << "\n";
    output << "// GUI Support: " << (useGUI ? "Enabled" : "Disabled") << "\n\n";
    
    // Define these BEFORE including main.cpp
    output << "#define CHOCO_EMBEDDED_MODE\n";
    if (!useGUI) {
        output << "#define CHOCO_NO_GUI\n";
    }
    output << "\n";
    
    output << "const char* EMBEDDED_SOURCE = R\"CHOCOSRC(\n";
    output << chocoSource;
    output << ")CHOCOSRC\";\n\n";
    
    // Include main.cpp (which now won't compile its own main() due to CHOCO_EMBEDDED_MODE)
    output << "#include \"main.cpp\"\n\n";
    
    // Now create our own main() function
    output << "int main(int argc, char* argv[]) {\n";
    
    if (useGUI) {
        output << "    ChocoGUI* gui = ChocoGUI::getInstance(argc, argv);\n";
        output << "    gui->setCallbackFunction(interpreterCallbackWrapper);\n";
    }
    
    output << "    try {\n";
    output << "        Lexer lexer(EMBEDDED_SOURCE);\n";
    output << "        std::vector<Token> tokens = lexer.tokenize();\n";
    output << "        Interpreter interpreter(tokens);\n";
    
    if (useGUI) {
        output << "        gui->setInterpreter(&interpreter);\n";
    }
    
    output << "        interpreter.execute();\n";
    output << "        return 0;\n";
    output << "    } catch (const LexerError& e) {\n";
    output << "        return 1;\n";
    output << "    } catch (const ParseError& e) {\n";
    output << "        return 1;\n";
    output << "    } catch (const RuntimeError& e) {\n";
    output << "        return 1;\n";
    output << "    } catch (...) {\n";
    output << "        std::cerr << \"Fatal error occurred\" << std::endl;\n";
    output << "        return 1;\n";
    output << "    }\n";
    output << "}\n";
    ...
    std::string outputFile = outputName.empty() ? getBaseName(inputFile) : outputName;
    outputFile += EXE_EXTENSION;
    
    // Get GTK flags if needed
    std::string gtkFlags;
    if (useGUI) {
#ifdef _WIN32
        // Windows: Redirect to temp file
        std::string tempFile = "temp_gtk_flags.txt";
        int pkgResult = system(("pkg-config --cflags --libs gtk4 > " + tempFile + " 2>nul").c_str());
        
        if (pkgResult == 0) {
            std::ifstream pkgOut(tempFile);
            if (pkgOut.good()) {
                std::getline(pkgOut, gtkFlags);
                pkgOut.close();
            }
        ...
        if (gtkFlags.empty()) {
            std::cout << "Warning: Could not get GTK4 flags via pkg-config" << std::endl;
            std::cout << "         Falling back to default paths..." << std::endl;
            // Fallback to common MSYS2 paths
            gtkFlags = "-IC:/msys64/mingw64/include/gtk-4.0 "
                      ...
                      "-lgtk-4 -lgraphene-1.0 -lgdk_pixbuf-2.0 -lcairo-gobject "
                      "-lcairo -lpangocairo-1.0 -lpango-1.0 -lharfbuzz "
                      "-lgdk-win32-4.0 -lgobject-2.0 -lglib-2.0";
        }
#else
        // Linux/Mac: Use popen
        FILE* pipe = popen("pkg-config --cflags --libs gtk4", "r");
        if (pipe) {
            char buffer[2048];
            std::string result;
    ...
    compileCmd << "g++ -o \"" << outputFile << "\" \"" << tempCpp << "\" ";
    
    if (useGUI) {
        compileCmd << "choco_gui.cpp ";
        if (!gtkFlags.empty()) {
            compileCmd << gtkFlags << " ";
        }
    }
    
    compileCmd << "-std=c++17 2>&1";
    
    std::cout << "Invoking C++ compiler..." << std::endl;
    ...
    if (result == 0) {
        ...
        if (!useGUI) {
            std::cout << "\nNote: This executable was compiled without GUI support." << std::endl;
            std::cout << "      GUI functions (gui_*) will not work." << std::endl;
        }
        ...
        std::cerr << "  - Make sure g++ is installed and in your PATH" << std::endl;
        
        if (useGUI) {
            std::cerr << "  - Make sure GTK4 development libraries are installed" << std::endl;
            std::cerr << "  - Try compiling without GUI: choco compile " << inputFile << " --no-gui" << std::endl;
        }
...
public:
    ...
    Value callFunction(const std::string& name, const std::vector<Value>& args, int callLine) {
        ...
        if (name.substr(0, 4) == "gui_") {
            throw RuntimeError("GUI function '" + name + "' not available - compiled without GUI support", callLine);
        }
#endif
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 2 && std::string(argv[1]) == "compile") {
        ...
        return compileChocoFile(inputFile, outputName, noGUI) ? 0 : 1;
    }
    ...
    if (argc == 1) {
        std::cout << "======================================" << std::endl;
        std::cout << "  CacaoLang 1.0.0 - Choco Crunch" << std::endl;
        std::cout << "  REPL (MochaInterpreter v1.0.1)" << std::endl;
        ...
        while (true) {
            ...
            if (line == "help") {
                std::cout << "CacaoLang Commands:" << std::endl;
